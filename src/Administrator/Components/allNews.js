...
export default function AllNews() {
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(getApiUrl(`/deleteNews/${id}`))
        .then(response => {
            Swal.fire({
                icon: "success",
                title: "Success...",
                ...
                >
                    ...
                    {galleries.map((gallery, index) => (
                        <article key={index} className="group h-full overflow-hidden rounded-lg border-2 border-gray-200 border-opacity-60 shadow-lg">
                            <img className="w-full transform object-cover object-center transition duration-500 ease-in-out group-hover:scale-105 md:h-36 lg:h-48" src={getMediaUrl(gallery.images)} alt="blog" />
