...
export default function UserListedFlats() {
    ...
    useEffect(() => {
        ...
        axios.get(getApiUrl(`/images/${userId}`))
            .then((response) => {
    ...
    if (isLoading) {
        return (
            <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                <p className="text-center text-base text-gray-500">Loading your listings...</p>
            </div>
        );
    }
    ...
    return (
        <section className="mx-auto max-w-7xl px-4 pb-6 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-2 pb-8 text-center sm:text-left">
                <h1 className="text-3xl font-bold text-gray-900 sm:text-4xl">Your Listed Flats</h1>
                <p className="text-sm text-gray-500 sm:text-base">
                    Review the rental flats you have listed and open checkout from any card.
                </p>
            </div>

            {galleries.length === 0 ? (
                <div className="rounded-3xl border border-dashed border-gray-300 bg-white px-6 py-12 text-center shadow-sm">
                    <p className="text-base font-medium text-gray-700">You have not listed any flats yet.</p>
                </div>
            ) : (
                            ...
                            <div className="space-y-5 p-5">
                                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-1 text-sm text-gray-500">Your listing is ready for booking and checkout.</p>
                                    </div>
                                    <button
                                        type="button"
